...
int CALLBACK wWinMain(const HINSTANCE hInstance, HINSTANCE, PWSTR lpCmdLine, const int nShowCmd)
{
	...
	wc.lpfnWndProc = [](HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)->LRESULT
	{
		...
		case WM_CREATE:
		{
			...
			OutputControl = CreateWindowExA(NULL, "static", nullptr,
				WS_CHILD | WS_VISIBLE | WS_TABSTOP,
				0, 0, 300, 50, hWnd, nullptr, nullptr, nullptr);
			...
			if (!possibly_isomorphic)
			{
				outfile << "Graph A and Graph B cannot be isomorphic because "
					...
					<< endl;
				SetWindowTextA(OutputControl, "Graph A and Graph B cannot be isomorphic because they have different sign frequency vectors in lexicographic order.");
			}
			if (possibly_isomorphic && !isomorphic)
			{
				...
				SetWindowTextA(OutputControl, "Graph A and Graph B have the same sign frequency vectors in lexicographic order but cannot be isomorphic.");
			}
			if (possibly_isomorphic && isomorphic)
			{
				outfile << "Graph A and Graph B are isomorphic." << endl << endl;
				SetWindowTextA(OutputControl, "Graph A and Graph B are isomorphic.");
	...
	const auto hwnd = CreateWindow(wc.lpszClassName,
		...
		315, 80, nullptr, nullptr, wc.hInstance, nullptr);
